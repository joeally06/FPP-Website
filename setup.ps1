...
if (Test-Path ".env.local") {
    ...
    if ($envContent -match "your-google-client-id" -or $envContent -match "GOOGLE_CLIENT_ID=\s*$") {
        Write-WarningCustom "Google OAuth credentials are not configured in your .env.local"
        if (Confirm-Action "Would you like to configure Google OAuth now?") {
            Write-Host ""
            ...
            Write-Success "Google OAuth credentials updated in .env.local"
            $skipOAuth = $false
        } else {
            Write-Info "Skipping OAuth configuration"
            $skipOAuth = $true
        }
    } else {
        Write-Success "Google OAuth appears to be configured"
        $skipOAuth = $false
    }
    
    # Check if other critical fields are missing
    if ($envContent -match "your-") {
        Write-WarningCustom "Some configuration values appear to be placeholders"
        Write-Host ""
        Write-Host "Current .env.local contains placeholder values like 'your-spotify-client-id'"
        Write-Host ""
        if (Confirm-Action "Review and update all configuration values?") {
            $backupName = ".env.local.backup-$(Get-Date -Format 'yyyyMMdd_HHmmss')"
            Move-Item ".env.local" $backupName
            Write-Info "Backed up old configuration to $backupName"
            $configureEnv = $true
        } else {
            Write-Info "Keeping existing configuration"
            Write-Host ""
            Write-WarningCustom "To manually update later, edit: .env.local"
            $configureEnv = $false
        }
    } else {
...
if ($configureEnv) {
    ...
    Write-Host ""
    Write-Info "Google OAuth is required for admin authentication"
    if (Confirm-Action "Do you have your Google OAuth credentials ready?") {
        Write-Host ""
        $googleClientId = Read-Host "Enter Google Client ID"
        $googleClientSecret = Read-Host "Enter Google Client Secret"
        $skipOAuth = $false
    } else {
        Write-WarningCustom "Google OAuth setup will be skipped"
        Write-Info "Admin login will NOT work until you configure OAuth"
...
# Ollama LLM (optional - configure later if needed)
